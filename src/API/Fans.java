...
public class Fans {
    ...
    //解析JSON,得到艺人id的集合
	 public static void jsonToObj_Artist_List(String jsonStr) throws Exception {   
	     ...
	     JSONArray jsonArray = resultObject.getJSONArray("performerList");
	    // System.out.println(totalCount);
	     
	     for(int i=0;i<jsonArray.length();i++){
	    	 JSONObject artist = jsonArray.getJSONObject(i);
	    	 String performerId=artist.optString("performerId"); 
	    	 int fansNum=Integer.parseInt(artist.optString("fansNum"));
	    	 if(fansNum>0){
	    	 artistList.add(performerId);
	     } else{
	    	 break;
	     }
	     }
	 ...
	 public static Set<String> getArtistList(int PageNum,String time,Fans fans,String sign) throws Exception{
	...
	//得到粉丝的ID,并写入文件
	public void jsonToObj_fans(String Filename,Set<String> set,String sign,String time) throws Exception{
		 File file_fans=new File(Filename);
		...
		for (String performId:set){
			...
			String jsonString_NEW = API.getJsonString(begin_url);
		     ...
		     String fatherName_NEW = jsonObject_NEW.getString("result");  
		     String result_NEW= AES.Aes.decrypt(fatherName_NEW);
		     ...
		     Fans_pageNum=API.getPageNum(begin_fans_jsonObj)/200+1;
			    for(int i=1;i<=Fans_pageNum;i++){
				    ...
				    // System.out.println(result);
				     JSONObject resultObject = new JSONObject(result);
				     String fansList = resultObject.optString("fansList");
				     //System.out.println(performId+":"+fansList);
				     out_fans.write(performId+"|"+fansList+"\n");
				}
		}
		     out_fans.close();
